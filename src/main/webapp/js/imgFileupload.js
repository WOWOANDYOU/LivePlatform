~(function(win){
	...
	ImgUploadeFiles.prototype = {
		...
		ready : function(){
			...
			this.imgcontent.innerHTML = '请上传不超过<b style="color:red">'+this.MAX+'</b>张'+_self.MW+' * '+_self.MH+'像素的图片';
		...
		fileBtn : function(c,f){
			...
			$(c).off().on('click',function(){
				debugger
				if(clickNum > this.MAX){
					layer.msg("图片不能超过"+MAX+"张",{icon:5});
					return false;
				}
				if(clickNum>0){
					$('.box').append("<input type='file' name='file' style='display:none;' class='file"+clickNum+"'>");
					f = $('.file'+clickNum)[0];
				}
				$(f).click();
				clickNum++;
	...
	ImgFileupload.prototype.init =function() {
		...
		this.dom.innerHTML = 
							'    <img src="/LivePlatform/images/login.gif" alt="" data-src="'+this.imgSrc +'" class="imsg">'+
							'    <i class="delImg">'+
							'        X'+
							'    </i>';
